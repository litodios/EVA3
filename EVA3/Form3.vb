...
Public Class Form3
    ...
    Private Sub btconsulta_clik(sender As Object, e As EventArgs) Handles btconsulta.Click
        Dim rut As String = txtrut.Text
        Dim correo As String = txtusuario.Text
        If String.IsNullOrWhiteSpace(rut) Then
            MessageBox.Show("Por favor, ingrese un RUT o corrreo para buscar datos.", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
        Using conexion As New MySqlConnection(connection)
            Try
                conexion.Open()
                Dim sql As String = "SELECT Rut, Correo, Contraseña, Tipo FROM usuarios WHERE usuarios = @Rut,@Correo"
                Using cmd As New MySqlCommand(sql, conexion)
                    ...
                    Using reader As MySqlDataReader = cmd.ExecuteReader()
                        If reader.Read() Then
                            ...
                            txtcontraseña.Text = reader("CONSTRASEÑA").ToString()
                            cmbtipo.Text = reader("tipo").ToString()
                        Else
                            MessageBox.Show("No se encontró ninguna persona con el RUT o ese Correo.", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        End If
                    End Using
                End Using
    ...
    Private Sub btagregar_click(sender As Object, e As EventArgs) Handles btagregar.Click
        ...
        Using conexion As New MySqlConnection(connection)
            Try
                ...
                Dim sql As String = "INSERT INTO  usuarios(rut, correo, contraseña, tipo) " &
                            "VALUES (@rut, @correo, @contraseña, @tipo)"
